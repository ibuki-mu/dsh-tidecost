window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		const PEAK_ENABLED_KEY = "dsh-balance.peakConfirm.enabled";
...
`;
		...
		*/
		...
		function readBoolLocal(key, def) {
		...
		*/
		...
		function apply(ctx) {
			ctx.effect(() => ctx.slots.inject("sidebar.footer.action", () => ctx.slots.register({
				...
				order: 90,
				label: () => "余额"
			}, BalanceTrigger)), "dsh-balance: sidebar trigger");
		}
		function BalanceTrigger({ wide, useSessions }) {
			...
			const [peakEnabled, setPeakEnabled] = (0, react.useState)(() => readBoolLocal(peakEnabledKey(), true));
			const [confirmedDate, setConfirmedDate] = (0, react.useState)(() => readLocal(peakDoneKey(beijingDateKey(Date.now())), null));
			...
			(0, react.useEffect)(() => {
				const check = () => {
					setNow(Date.now());
					const done = readLocal(peakDoneKey(beijingDateKey(Date.now())), null);
												...
												/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
													...
													children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
														...
														onChange: (e) => {
															...
															writeLocal(peakEnabledKey(), v ? "1" : "0");
															setPeakEnabled(v);
														}
													}), "启用"]
												}),
