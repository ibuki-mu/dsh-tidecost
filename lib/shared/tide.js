...
// ── 价格纪年常量（官方公告口径）───────────────────────────────────────────
const FLASH_LEGACY = { cacheHit: 0.019, cacheMiss: 0.95, output: 1.91 };
const FLASH_ERA1_VALLEY = { cacheHit: 0.05, cacheMiss: 1.5, output: 4.5 };
const FLASH_ERA1_PEAK = { cacheHit: 0.10, cacheMiss: 3.0, output: 9.0 };
/** 2026-09-10 12:00 北京起：Flash 系列新价。 */
const FLASH_ERA2_VALLEY = { cacheHit: 0.02, cacheMiss: 1, output: 4 };
const FLASH_ERA2_PEAK = { cacheHit: 0.04, cacheMiss: 2, output: 8 };
const PRO_LEGACY = { cacheHit: 0.02, cacheMiss: 2.97, output: 5.93 };
const PRO_VALLEY = { cacheHit: 0.15, cacheMiss: 4.5, output: 13.5 };
const PRO_PEAK = { cacheHit: 0.30, cacheMiss: 9.0, output: 27.0 };
...
export function priceEntryFor(model) {
    ...
    if (id.length > 0) {
        ...
        for (const key of keys) {
            if (id === normalizeModel(key))
                return PRICE_TABLE_CNY[key];
        }
        for (const key of keys) {
            const nk = normalizeModel(key);
            if (id.includes(nk) || nk.includes(id))
                return PRICE_TABLE_CNY[key];
        }
        if (id.includes('pro'))
            return PRICE_TABLE_CNY['deepseek-v4-pro'];
...
export function toCny(amount, currency, usdCny = USD_CNY_DEFAULT) {
    if (currency !== 'USD')
        return amount;
...
export function inferProvider(provider, model) {
 ...
 */
export function flatPricesFor(provider, model) {
    ...
    for (const [key, entry] of Object.entries(table)) {
        if (key !== '*' && (m === normalizeModel(key) || m.includes(normalizeModel(key))))
            return entry;
    }
    return table['*'] ?? null;
}
/** 一步的档位标签：按量计费 provider → flat；否则走 DeepSeek 峰谷档位。 */
export function stepTier(provider, model, atMs, holidays = []) {
    if (flatPricesFor(provider, model))
        return 'flat';
...
/** 是否为周末（UTC 自然日周六/周日）。 */
export function isUtcWeekend(atMs) {
    const day = new Date(atMs).getUTCDay();
    return day === 0 || day === 6;
}
/** 是否为配置的节假日（北京日期名单）。 */
export function isHoliday(atMs, holidays = []) {
 ...
 */
export function isPeakAt(atMs, holidays = []) {
    if (isUtcWeekend(atMs))
        return false;
    if (isHoliday(atMs, holidays))
        return false;
...
export function tierAt(atMs, holidays = []) {
    if (Number.isFinite(atMs) && atMs < LEGACY_BOUNDARY_MS)
        return 'legacy';
 ...
 */
export function phaseAt(atMs, holidays = []) {
    ...
    for (const p of points) {
        if (p.at <= atMs && (prev === null || p.at > prev.at))
            prev = p;
        if (p.at > atMs && (next === null || p.at < next.at))
            next = p;
    }
    return {
...
export function priceCny(model, atMs, holidays = []) {
    ...
    if (atMs < LEGACY_BOUNDARY_MS)
        return epoch.valley; // legacy 区间 valley === peak
 ...
 */
export function costCny(tokens, model, atMs, holidays = [], provider, usdCny = USD_CNY_DEFAULT) {
...
export function beijingScheduleSegments() {
    return [
 ...
 */
export function currentBeijingSegmentIndex(atMs, holidays = []) {
    if (isUtcWeekend(atMs) || isHoliday(atMs, holidays))
        return null;
    ...
    for (let i = 0; i < segs.length; i += 1) {
        ...
        if (s.toBeijingHour > 24) {
            if (bjHour >= s.fromBeijingHour || bjHour < s.toBeijingHour - 24)
                return i;
        }
